...
Now build the Project. (One time only)
```{r}
...
# Bug with creating arrow files on ArchR 1.0.2 (something to do with parallel HD5 Writing).
...
projNC <- addMotifAnnotations(
  ...
  name = "Motif", collection = "CORE", force = T)
...
Motif_list <- getMarkers(Cluster_Motifs, cutOff = "FDR <= 0.01 & Log2FC >= 1.25")
Motif_df <- as.data.frame(Motif_list)

top_Motifs <- Motif_df %>% dplyr::group_by(group_name) %>% dplyr::top_n(3, wt = Log2FC)

motifs <- c("NKX6.3", "SOX9", "SNAI2", "NR2F2", "TEAD1", "MEIS1.var.2",
            "GATA1..TAL1","TFAP2B.var.3","mix.a","BHLH34","ACE2",
            "TFAP2A","EBF1","HOXB6","GBX2","Sox17","Six3","Pou5f1..Sox2")
...
```{r}
# C7 is HH6/root
trajectory1 <- c("C6", "C2") # Sensory neurons
trajectory2 <- c("C6", "C5") # Sensory Ganglia
trajectory3 <- c("C6", "C1") # Melanocytes
trajectory4 <- c("C6", "C4","C3") # Mesenchyme
trajectory5 <- c("C6", "C8") # Not Sure
trajectory6 <- c("C6", "C9", "C10") # Neural
...
C3_cells <- projNC@cellColData[projNC@cellColData$Clusters == "C3",]
C8_cells <- projNC@cellColData[projNC@cellColData$Clusters == "C8",]
...
C8_weighted_age <- avg_hours(C8_cells)
...
C3END_Weighted <- as.numeric(projNC@cellColData[,"C3END"] * C3_weighted_age)
C8END_Weighted <- as.numeric(projNC@cellColData[,"C8END"] * C8_weighted_age)
