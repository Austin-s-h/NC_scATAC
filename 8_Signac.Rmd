...
for (s in c("HH6", "HH8", "HH10", "HH12", "HH14", "HH16", "HH18")) {
  ...
  print(paste0("Reading ", fragpath, " and ", input_csv_file))
  metadata <- read.csv(file = input_csv_file, header = TRUE, row.names = 1)
  print(paste0("Creating Seurat object from", nrow(metadata), 
    " unfiltered cells cells. After 5000 bin QC, there are ", 
    length(barcode_names), "cells remaining."))
  # Create a fragment object
  frags <- CreateFragmentObject(path = fragpath, cells = barcode_names)
  counts <- FeatureMatrix(fragments = frags, features = combined_peaks, 
    cells = barcode_names)
  # Create a chromatin object
  assay <- CreateChromatinAssay(counts = counts,
...
combined <- FindTopFeatures(combined, min.cutoff = 'q20')
...
umap <- DimPlot(object = combined, label = TRUE, reduction = "umap") + NoLegend()
ggsave(umap, filename = paste0("export/", s, "combined_postfilter_UMAP.pdf"), width = 8, height = 8)
```