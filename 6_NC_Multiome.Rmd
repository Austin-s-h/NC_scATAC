...
```{r}
input_files <- getInputFiles(paths=c("/workspaces/NC_scATAC/NC_Multiome/outs/"))
input_files <- input_files["atac_possorted"]
...
arrow_files <- createArrowFiles(
  ...
  minTSS = 0.15,
  minFrags = 3500,
  maxFrags = 500000,  # maybe 400K even
...
mat <- assay(seRNA)
colnames(mat) <- gsub("atac_possorted#|gex#","",colnames(assay(seRNA)))
assay(mat, withDimnames=FALSE) <- mat

noprefix <- gsub("atac_possorted#|gex#","",proj$cellNames)
proj$noprefix <- noprefix
subset <- which(proj$noprefix %in% neuron.list)
