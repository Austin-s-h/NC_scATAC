...
rownames(markersGS) <- markersGS@elementMetadata$name

strong_GS_markers <- as.data.frame(getMarkers(markersGS, cutOff = "FDR <= 0.05 & Log2FC >= 0.5"))

# Focus Labels
GSMarkerLabel <- c("TWIST1")
...
heatmap10 <- ComplexHeatmap::Heatmap(
  matrix = ordered_neural3[unique(melano_motif_df$name),],
  cluster_rows = T, cluster_columns = F, show_column_names = F,
  show_row_names = F, use_raster = F, row_title = "chromVAR z-score",
  column_title = "Cells ordered in Pseudotime: Melanocytes",
  col = as.character(ArchRPalettes$blueYellow)) + 
...
unique(melano_motif_df$name)[row_order(heatmap8)]


plotPDF(heatmap10, name = "Plot-.pdf", ArchRProj = projNC, addDOC = FALSE, width = 6, height = 5)
...
```
# Let's interactively explore with the ArchRBrowser
```{r}
ArchRBrowser(projNC)
```
