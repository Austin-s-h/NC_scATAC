...
Highlighting the mesencyhme, neural 1, neural 2, and sensory ganglia lineages.
First we have to isolate the cells of that lineage to perform the analysis on.
```{r}
...
plotPDF(heatmap3, name = "Plot-Mesenchyme-Motif-Pseudotime-Heatmap.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 10)
...
which(projNC$cellNames %in% neural1_cells)
...
labels <- c("RFX_Family","Pou5f1..Sox2_38","ZIC_Family","RBPJL",
            "SCRT2","SNAI_Family","LHX4","TBX1","MEIS2",
            "EOMES","PBX3","SOXE_Family")
...
labels <- c("RFX1_724","Pou5f1..Sox2_38","ZIC1_190",
            "Rbpjl_585","SCRT2_728","SNAI2_729","Lhx4_194",
            "MEIS2_262","PBX3_401","SOX10_407","POU3F1_274",
            "RFX2_283","TBX1_295","EOMES_290")
label_index <- which(unique(neural1_motif_df$name) %in% labels)
label_text <- stringr::str_split(unique(neural1_motif_df$name)[label_index], pattern = "_", simplify = T)[,1]


heatmap4 <- ComplexHeatmap::Heatmap(
  matrix = ordered_neural1[unique(neural1_motif_df$name),],
  cluster_rows = T, cluster_columns = F, show_column_names = F,
  show_row_names = F, use_raster = F, row_title = "chromVAR z-score",
...
plotPDF(heatmap4, name = "Plot-neural1-Motif-Pseudotime-Heatmap.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 10)
...
plotPDF(heatmap5, name = "Plot-neural2-Motif-Pseudotime-Heatmap.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 10)

heatmap5
```

```{r}
# Cell names along sensory ganglia trajectory
...
plotPDF(heatmap6, name = "Plot-sensory-Motif-Pseudotime-Heatmap.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 10)
...
# Add if colorBy = "MotifMatrix" to pass in useSeqnames = "z" by default.
# plotTrajectoryCustom <- edit(plotTrajectory)

# Genescores too
genes <- c("GRHL3","ASCL1","DLX5")
plotting_genes <- getFeatures(projNC, select = paste(genes, collapse="|"), useMatrix = "GeneScoreMatrix")

# p10 <- plotTrajectory(projNC, trajectory = "C2END", colorBy = "GeneScoreMatrix", name = plotting_genes, continuousSet = "blueYellow", plotAs = "points", addArrow = F, embedding = "UMAP")

#Now we gotta do some individual line plots.
library(reshape2)
data <- total_dev_mes3[rownames(total_dev_mes3) %in% top_differential_motifs_df$name,]
rownames(data)

data <- melt(data)
data$Pseudotime <- qdapTools::lookup(data$Var2, lookup)

data_sub <- subset(data, data$Var1 %in% c("SOX2","SOX9","RXRG","TWIST1","ZIC5"))

# order by cell
data_sub <- data_sub %>%
    arrange(match(Var2, as.character(lookup2$cell)))


ggplot(data_sub, aes(x = Var2, y = as.numeric(value),
                     color = Pseudotime, group = Var1)) + 
  geom_point() + 
  ...
  xlab("Cells in Pseudotime") +
  ylab("ChromVAR Deviation Score")
...
```{r}
motif_to_hi <- c("SOX8", "TWIST1","RFX2","ZIC1","OTX2","Dmbx1","YY2","RFX3","GRHL3")
...
combined <- cowplot::plot_grid(motif_highlights[[1]],gene_highlights[[8]],
                               motif_highlights[[7]],gene_highlights[[9]],
                               motif_highlights[[3]],gene_highlights[[7]],
                               motif_highlights[[5]],gene_highlights[[1]],
                               ncol = 2)

combined2 <- cowplot::plot_grid(motif_highlights[[6]],gene_highlights[[6]],
                               motif_highlights[[8]],gene_highlights[[2]],
                               motif_highlights[[9]],gene_highlights[[5]],
                               ncol = 2)
plotPDF(combined, name = "Plot-Motif-GeneScore-UMAP-Projection-1.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 12)
plotPDF(combined2, name = "Plot-Motif-GeneScore-UMAP-Projection-2.pdf", ArchRProj = projNC, addDOC = FALSE, width = 8, height = 12)
```
...
p11 <- ggplot(data.frame(corGSM_MM), aes(cor, maxDelta, color = TFRegulator, label = label)) +
  geom_point() + 
  theme_ArchR() +
...
plotPDF(p11, name = "Plot-GeneScore-TF-Correlation.pdf", ArchRProj = projNC, addDOC = FALSE, width = 6, height = 7)
